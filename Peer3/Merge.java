...
public class Merge extends Thread {
    ...
    private static void mergeFile(String[] filearray){
        try{
            ...
            for(int i = 0; i < lengthofArray; i++){
                 f[i] = new File("Peer3FileSplit/"+filearray[i]);
            }
           
            File fMerged = new File("Peer3MergedFile/mergedFile");
 ...
 //to do merging stuff here
            ...
            out.close();
            System.out.println("File is merged and available for Peer3.");
        }
        catch(Exception ex)
        {
    ...
    public void run() {
        ...
        fileList = readChunkName.readFileName("src/Peer3/fileparts.txt");
        String [] fileArray = new String[fileList.size()];
        fileArray=fileList.toArray(fileArray);
        chunkSizeList=readChunkName.readFileName("src/Peer3/filesize.txt");
        ...
        while(true){
            ...
            for(int i = 0; i < fileList.size(); i++){
                File f=new File("Peer3FileSplit/"+fileArray[i]);
                if(!f.exists())
                {
               	 Thread.sleep(2000);
                }
...
//                }
         ...
         mergeFile(fileArray);
         System.out.println("The file has been transfered successfully to Peer3");
       }
    	catch(Exception ex)
    	{
    		ex.getMessage();
