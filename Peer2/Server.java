...
public class Server
{
    public static void main(String args[]) throws Exception
    {
    	...
    	*/
        ...
        uploadFileList = readChunkName.readFileName("src/Peer2/initialchunks.txt");
        ...
        chunkFileList = readChunkName.readFileName("src/Peer2/fileparts.txt");
...
class Handler extends Thread
{
    ...
    {   
    	//System.out.println("Entered transferfiletopeer method");
    	int val;
        ...
        while(true){
            ...
            if(f.exists()&&((int)f.length()==(Integer.parseInt(chunkSizeArray[0]))||((int)f.length()==Integer.parseInt(chunkSizeArray[1])))){ 
             //if(f.exists() && (f.length() == chunkSizeArray[0].length() || f.length() == chunkSizeArray[1].length()) )
            	{
            		File file = new File("Peer2FileSplit/");
