...
public class Merge extends Thread {
    ...
    private static void mergeFile(String[] filearray){
        try{
            ...
            for(int i = 0; i < lengthofArray; i++){
                 f[i] = new File("Peer4FileSplit/"+filearray[i]);
            }         
            File fMerged = new File("Peer4MergedFile/mergedFile");
 ...
 //to do merging stuff here
            ...
            out.close();
            System.out.println("File is merged and available for Peer4.");
        }
        catch(Exception ex)
        {
    ...
    public void run() {
        ...
        fileList = readChunkName.readFileName("src/Peer4/fileparts.txt");
        ...
        while(true){
            ...
            for(int i = 0; i < fileList.size(); i++){
                f=new File("Peer4FileSplit/"+fileArray[i]);
                if(!f.exists())
                {
               	 Thread.sleep(2000);
                }
...
//                }
         ...
         mergeFile(fileArray);
         System.out.println("The file has been transfered successfully to Peer4");
       }
    	catch(Exception ex)
    	{
    		ex.getMessage();
